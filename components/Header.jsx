...
function Header({ setOnSignUp, onSignUp, onSignIn }) {
    ...
    return (
        <>
            <div id='header' className={`header ${(onSignUp || onSignIn) ? 'backdrop-blur-sm' : ''} w-[360px] h-[236px] mb-[66px] bg-[url('../public/assets/cardimage/headerbg.png')] bg-no-repeat bg-cover`}>
                <div className='absolute w-[16px] h-[16px] left-[20px] top-[20px] text-white'><Image src={back_arrow} alt='' /></div>

                {/* Join Group  */}
                <div onClick={() => setOnSignUp(true)} className='cursor-pointer w-[76px] h-[28px] absolute top-[14px] left-[266px] rounded-[4px] text-white box-border border-[0.8px] border-solid border-white'><span className='absolute w-[60px] h-[16px] top-[6px] left-[8px] font-[600] text-[12px] leading-[16px] text-white font-IBM'>Join Group</span></div>
